...
#define syntax2 \
" id=sl gimp:layer-mode layer-mode=screen blend-space=rgb-perceptual aux=[ ref=sl softglow glow-radius=50  brightness=0.42 sharpness=0.357  gimp:layer-mode layer-mode=overlay blend-space=rgb-perceptual opacity=0.71 aux=[ color value=#afafaf ] crop aux=[ ref=sl ] id=gaus gimp:layer-mode layer-mode=luminance opacity=0.65 aux=[ ref=gaus gaussian-blur abyss-policy=none clip-extent=false std-dev-x=8 std-dev-y=8 ] levels in-high=1.2 out-high=1.0 "\
...
default: blendmode = state->softlight;
  ...
  gegl_node_link_many (state->input, state->graph0, state->graph1, state->hardlight, state->crop, state->graph2, state->nop, blendmode, state->output, NULL);
